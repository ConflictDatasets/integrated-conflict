...
author: "Tom Brailey^[Original author: Micol Morellini.]"
date: "10/27/2021" 
output:
...
Subsetting the SCAD dataset to years between 1997 and 2017 results in 175 observations.
...
We develop the actor taxonomies separately for each conflict dataset. The initial recoding process relies on existing research, existing databases, and online resources. We then use the taxonomy developed by @Donnay2019 to standardise actors across datasets. This can be found in the file `summary.rmd` and on our OSF page.
...
We base our event taxonomy on @Donnay2019. Again, this file is provided on our OSF page.
...
We use the precision taxonomy developed by @Donnay2019. Again, this file is provided on our OSF page.
...
## Comparing flagged and de-duplicated events across specifications
...
flagged_event_km5_dfs <- NA
flagged_episode_km5_dfs <- NA

for(i in 1:length(flagged_km5)){
  flagged_event_km5_dfs[i] <- flagged_km5[[i]]$`Flagged Event Information`$data.source
  flagged_episode_km5_dfs[i] <- flagged_km5[[i]]$`Flagged Episode Information`$data.source
}

table(flagged_event_km5_dfs)

table(flagged_episode_km5_dfs)
...
While here we see an examples of a flagged instance where the events, again coming from ACLED and GED, turned out not to be duplicates:
...
cdf4$enddate <- as.Date(cdf4$enddate, origin = "1970-01-01")
...
We also check that the our previous taxonomies (event, precision, and actor) cover all observations in this merged dataset: 
...
write.csv(cdf4, here::here("data", "ethiopia_conflict.csv"))
saveRDS(cdf4, here::here("data", "ethiopia_conflict.rds"))
haven::write_dta(cdf4, here::here("data", "ethiopia_conflict.dta"))
haven::write_sav(cdf4, here::here("data", "ethiopia_conflict.sav"))
